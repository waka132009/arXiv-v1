...
def build_pdf_page_2_flowchart(outfile: Path):
    ...
    # --- START: User's provided layout code (v9) ---
    
    # --- 数式用背景ボックス ---
    box_y_top = 0.88
    box_height = 0.14 # 0.13 -> 0.14
    ...
    # --- 数式 (User's fontsize=14 and Y-coords) ---
    ...
    ax_main.text(0.5, box_y_top - 0.13, # Y=0.75 (User: -0.13, Y=0.75)
                 r"$f_{\rm eq} + f_{\rm BZ} + f_{\rm res} = 1$",
                 ha='center', va='top', fontsize=14, transform=ax_main.transAxes)
    ax_main.text(0.5, box_y_top - 0.18, # Y=0.70 (User: -0.18, Y=0.70)
                 r"The fractional contributions sum to unity.",
                 ha='center', va='top', fontsize=9, linespacing=1.4, transform=ax_main.transAxes)
    
    # --- 説明文 (User's Y-coord) ---
    ax_main.text(0.5, box_y_top - box_height - 0.09, # Y=0.65 (User: -0.09)
                 "An equatorial, unipolar-induction spin-extraction channel taps the black hole’s rotational energy\n and re-injects a finite share into the disk, creating a self-sustained yet self-limiting cycle.",
    ...
    gs_flowchart = fig.add_gridspec(nrows=2, ncols=3,
                                    wspace=0.35, hspace=0.3, # User: 0.3
    ...
    # --- Row 1: Boxes 1, 2, 3 (User's text) ---
    ax_flow1 = fig.add_subplot(gs_flowchart[0, 0])
    ax_flow1.axis("off")
    ax_flow1.text(0.5, 0.4, "1. High accretion —\nInner-disk inflow reaches\nsuper-Eddington rates.",
    ...
    ax_flow3.axis("off")
    # (User's fix: wrap=True removed)
    ax_flow3.text(0.5, 0.4, r"3. Sustained $L/L_{\rm Edd} > 1$ —" "\n"
                  r"Reinjected power" "\n"
                  r"sustains the disk/corona" "\n"
                  r"luminosity.",
                  ha='center', va='center', fontsize=9, linespacing=1.3, bbox=box_props_123, transform=ax_flow3.transAxes)

    # --- Row 2: Box 4 (User's text) ---
    ax_flow4 = fig.add_subplot(gs_flowchart[1, 1]) # 中心のセル
    ax_flow4.axis("off")
    ax_flow4.text(
        0.5, 0.5,
        "4. Spin regulation (feedback) —\n"
        "Counter-torque reduces $a_\\ast$, \n"
        "enforcing a spin ceiling\n"
        "and self-limiting the excitation.",
        ha="center", va="center",
        fontsize=9, linespacing=1.3,
        bbox=box_props_fb,
        wrap=False,                        # ← 改行を固定したい場合
        transform=ax_flow4.transAxes
    )    
    # --- Arrows (FIX: 矢印の座標を wspace の変更に合わせて調整) ---
    # 1 -> 2 (Straight)
    ax_main.add_patch(FancyArrowPatch((0.24, 0.48), (0.30, 0.48), # User (0.28->0.34)
                                      arrowstyle='->', mutation_scale=15, color='black', lw=0.8, transform=fig.transFigure))
    # 2 -> 3 (Straight)
    ax_main.add_patch(FancyArrowPatch((0.65, 0.48), (0.72, 0.48), # User (0.62->0.68)
                                      arrowstyle='->', mutation_scale=15, color='black', lw=0.8, transform=fig.transFigure))
    # 2 -> 3 の直線（右向き：ジェット側）＝ P_BZ (axial)
    ax_main.text(0.685, 0.501, r"$P_{\rm BZ}$ (axial)",
             ha="center", va="bottom", fontsize=8, color="#334155",
             transform=fig.transFigure)
    
    # 3 -> 4 (Down-left curve) (User's coords)
    ax_main.add_patch(FancyArrowPatch((0.75, 0.46), (0.65, 0.20), 
                                      arrowstyle='->', mutation_scale=15, color='black', lw=0.8, transform=fig.transFigure,
                                      connectionstyle="arc3,rad=-0.3"))
    # ②→④ のカーブラベル（Peq）
    ax_main.text(0.690, 0.355, r"$P_{\rm eq}$ (diskward)",
             ha="center", va="center", fontsize=8, color="#334155",
             transform=fig.transFigure)
    
    # 4 -> 2 (U-turn curve) (User's coords)
    ax_main.add_patch(FancyArrowPatch((0.40, 0.32), (0.35, 0.26), 
                                      arrowstyle='->', mutation_scale=15, color='black', lw=0.8, transform=fig.transFigure,
                                      connectionstyle="arc3,rad=0.8"))
    
    # --- END: User's provided layout code ---
