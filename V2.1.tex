...
\FloatBarrier
\phead{Primary falsifier (decision rule).---}We treat SSPE as a population-level, protocol-driven claim rather than a single-object diagnostic.

In vetted high-spin, high-$\lambda$ samples, absence of $\ge 2$ independent hooks after the transparency/absorption/inclination screens refutes the framework \emph{within the testable domain}.
\smallskip
\phead{Additional falsifiers (secondary).---}Beyond the primary decision rule (Sec.~\ref{sec:obs}), the following are \emph{supporting} population-level patterns that, if established robustly, would further disfavor a non-zero equatorial fraction $f_{\rm eq}$:
