...
\begin{tenumerate}
  ...
  \item \textbf{Next (Sec.~\ref{sec:hooks}):} We translate the framework into an operational observing protocol with minimal Screens (A--D) and three joint hooks (spectral, timing, polarimetric) with explicit pass/fail criteria (Table~\ref{tab:hook-spec}).
  \item \textbf{Decisive test (Sec.~\ref{sec:discussion}):} In vetted high-spin/high-$\lambda$ samples, failure to recover $\ge 2$ hooks within a representative epoch (missing outcomes excluded by construction) falsifies the framework.
\end{tenumerate}
...
We provide an audit-ready candidate-identification and falsification protocol within a screened domain; questions of universality and prevalence are left to follow-up work.
...
If an observable is effectively inaccessible at useful S/N—i.e., it cannot yield a decidable pass/fail under the fixed Tier-1 floors and decision rule in Table~\ref{tab:hook-spec} (given the stated uncertainties/limits)—treat it as \emph{missing} rather than \emph{failed}.
...
To prevent post-hoc reclassification, we predefine an operational floor for each Hook in Table~\ref{tab:hook-spec}; below this floor the outcome is \emph{missing} by construction and does not count toward falsification.
...
\enddata
\tablecomments{
After applying Screens~A--D, each hook may have degeneracies in isolation; SSPE is evaluated by joint coherence across Hooks~1--3. We adopt $\ICmetric \ge \ICcrit$ as a pragmatic strong-evidence decision threshold (with $\AICcmetric \ge \ICcrit$ as a secondary check) following common usage in model selection (e.g., \citealt{KassRaftery1995}; \citealt{BurnhamAnderson2002}); Failure to recover $\ge 2$ \emph{decidable} hooks contributes to the population-level falsification tally; see Appendix~\ref{app:protocol}, Eqs.~(\ref{eq:PF1}--\ref{eq:PF2}).
\\[1ex]
...
\\[1ex]
Table~\ref{tab:hook-spec} defines per-hook adjudication (pass/fail/missing), while Appendix~A defines the object-level tally and the population-level trigger (Eqs.~\ref{eq:PF1}--\ref{eq:PF2}).
}
\tablenotetext{a}{An \emph{epoch} is operationally defined in Appendix~\ref{app:protocol}.}
...
\FloatBarrier
\pheadl{ph:primary-falsifier}{Primary falsifier (population-level).}SSPE is refuted if at least $N_{\min}$ \emph{independent objects that survive all Screens~A--D}(i.e., the vetted sample) yield \emph{decidable} joint-hook outcomes, and more than a fraction $f_{\rm fail}$ of those objects fail to recover $\ge 2$ hooks \emph{within the same epoch}.
...
\noindent\emph{Operational trigger.—} The executable Tier-1 population decision is defined in Appendix~\ref{app:protocol} (Eqs.~\ref{eq:PF1}--\ref{eq:PF2}) and is the rule used for all Tier-1 bookkeeping in this work.
...
%\phead{Population-level falsification (Tier-1).} For Tier-1 we adopt a one-sided significance level of $\alpha = 0.05$.
%Any ($N_{\min}, f_{\rm fail}$) phrasing elsewhere is a heuristic summary for a given ($N_{\rm eff}, \alpha$) .

\phead{Single source of truth (Tier-1).—}  For Tier-1 we adopt a one-sided significance level of $\alpha = 0.05$; Eqs.~(\ref{eq:PF1}--\ref{eq:PF2}) define the operational population-level trigger; any $(N_{\min}, f_{\rm fail})$ phrasing elsewhere is a heuristic summary for a given $(N_{\rm eff},\alpha)$.
...
If a Tier-1 hook cannot be adjudicated as pass/fail at useful S/N under the fixed floors in Table~\ref{tab:hook-spec} (given the stated uncertainties/limits), its outcome is treated as missing by construction (not failed) and is excluded from the population-level falsification tally (i.e., it does not count as a failure).
...
The Tier-1 pass/fail tally is evaluated using the population-level decision rule; see Appendix~\ref{app:protocol}, Eqs.~(\ref{eq:PF1}--\ref{eq:PF2}).
