...
Larger measurement uncertainties can make the constancy test easier to satisfy, but this does not strengthen the evidence: if a Tier-1 hook cannot be adjudicated as pass/fail at useful S/N under the fixed floors in Table~\ref{tab:hook-spec} (given the stated uncertainties/limits).

In that case, its outcome is treated as \emph{missing by construction} (not failed) and is excluded from the population-level falsification tally (i.e., it does not count as a failure).
