...
First, we formalize the geometry and kinematics of the ISCO--ergoregion interface and introduce a minimal parameterization of the Penrose-like coupling, $\epsilon_{\rm coup}(a_\ast)$, and the spin threshold $a_{\rm th}$ (Section~\ref{sec:theory}).
...
Fourth, we apply the SSPE framework to jet-dominated quasars whose disks appear anomalously extended and hot, arguing that equatorial feedback from SSPE can naturally inflate and overheat the outer disk even in BZ-favorable systems (Section~\ref{sec:power}).
