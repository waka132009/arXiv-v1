...
\noindent\textbf{Screen B ($\lambda$):} require an Eddington-ratio estimate and select high-$\lambda$ systems (within stated systematics).\\
\noindent\textbf{Screen C (geometry/obscuration):} exclude Compton-thick absorption and extreme inclination; treat an affected hook as missing only if plausibly blocked.\\
\noindent\textbf{Screen D (compactness):} require $\ell<\ellcrit$ (fiducial $\ellcrit=30$).\\[2pt]
