...
We measure energy-resolved lags $\tau(E)$ relative to a fixed \emph{reference} band $E_{\rm ref}$ (we adopt $E_{\rm ref}=2$--4~keV unless stated otherwise),
...
\tableline
Hook 2 & Lag hardening & lag--energy slope & \mccell{$\tau(E)$ relative to $E_{\rm ref}=2$--4~keV \\ define $\tau>0$ for lags; \\ pass if $d\tau/d\log E>0$ \\ and monotone within an epoch.} & dilution\\
\tableline
...
\noindent\textbf{Screens A--D (defined in Sec.~\ref{sec:obs}; summarized here)} \\
Exclude Compton-thick, extreme inclination; require $\ell<\ellcrit$.\\
