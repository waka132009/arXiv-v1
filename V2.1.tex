...
\tableline
Hook 2 & \mcell{Energy-resolved X-ray light curves\\(multiple bands; sufficient counts)\\+ lag--energy spectrum $\tau(E)$\\(relative to a stated $E_{\rm ref}$)\\\textem{Tier-2:} X$\to$UV/optical reprocessing lags\\where available} & \textbf{Partly}\tablenotemark{a} & \mcell{After screens, lag--energy slopes\\fail to be $d\tau/d\log E>0$ and monotone within an epoch\\across the bright subset}\\
\tableline
Hook 3 &
...
\tablehead{
  ...
  \colhead{$M_{\rm BH}/M_\odot$} & \colhead{$L/L_{\rm Edd}$} & \colhead{Arch. (Hook~1/2/3)} & \colhead{Notes}
}
\startdata
