...
Its outcome is treated as \emph{missing by construction} (not failed) and is excluded from the population-level falsification tally (i.e., it does not count as a failure).

\phead{Decision (Tier-1 bookkeeping).}
The Tier-1 pass/fail tally is evaluated using the population-level decision rule; see the Primary falsifier (population-level) on p.~\pageref{ph:primary-falsifier}.
