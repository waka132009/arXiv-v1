...
For the fiducial coupling used in Eq.~(Eqs.~\eqref{eq:dM}--\eqref{eq:da}), the characteristic drift toward $a_{\rm eq}<1$ occurs over $t_{\rm act}\sim{\rm Myr}$--tens of Myr (Fig.~\ref{fig:spin-down}), implying $t_{\rm real}\sim (t_{\rm act}/d)$.
...
\end{titemize}
\noindent\textbf{Lag convention.}
We measure energy-resolved lags $\tau(E)$ relative to a fixed \emph{reference} band $E_{\rm ref}$ (we adopt $E_{\rm ref}=2$--4~keV in the rest frame unless stated otherwise; in observed frame use Eref/(1+z) where feasible),
