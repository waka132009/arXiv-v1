...
In SSPE the heating channel is ultimately powered by black-hole rotational energy extraction, so the joint-hook strength is expected to correlate with spin-linked quantities at fixed $(\MBH,\lambda)$.
...
\item $L_{\rm bol}$ not anti-correlated (even weakly) with $a_\ast$ within matched $(\MBH,\lambda)$ bins for the bright subset (no self-regulated spin-down imprint).\\ To prevent post-hoc bin-shifting, we define matched bins using pre-specified fixed logarithmic intervals (e.g., 0.5 dex in both $\MBH$ and $\lambda$) and report sensitivity to reasonable alternative bin widths.
