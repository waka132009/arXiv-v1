...
%Failure to trigger Tier-1 falsification is not evidence for SSPE; it is treated as inconclusive within the present protocol.

%High raw failure fractions ($f_{\rm fail}$) are still reported and interpreted as tension with the scenario, even when the formal Tier-1 trigger is not met for the current effective sample size.
...
We adopt $a_\ast \ge 0.95$ as an operational high-spin criterion, allowing margin for systematic uncertainties in spin estimators relative to the phenomenology-implied provisional activation scale ($a_\ast \simeq 0.97$), to be calibrated observationally.
...
(iii) a hard-component EVPA preferentially aligned with the disk plane within the screened high-$a_\ast$, high-$\lambda$ subset typically requires fine-tuned coronal geometry and scattering configurations.
...
Instead, after applying Screens A--D, it asks whether the SSPE-like joint-hook pattern is recovered in more than one screened source in the archival sample.\\
If two or more screened sources satisfy the joint-hook pass condition, the existence-style claim is established at Tier-1.\\
If \emph{zero or one} such source remains, the existence-style claim is not established.\\
If the number of screened or decidable cases is too small, or if missing outcomes dominate because of data limitations, the archival Tier-1 result is treated as inconclusive rather than negative.\\
...
Beyond the primary Tier-1 criterion, the following patterns, while individually non-decisive and potentially confounded, would further weaken the case for a non-zero equatorial fraction $f_{\rm eq}$:
...
\end{enumerate}
If \emph{two or more} items above hold simultaneously for the same high-spin, high-$\lambda$ subset, would strengthen the case for $f_{\rm eq}\!\to\!0$ and reduce the motivation for invoking this mechanism in that subset.
