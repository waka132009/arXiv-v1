...
\begin{tenumerate}
  ...
  \item \hyperref[list:screens]{Screens S1–S4} remove confounders: Compton-thick absorption, extreme inclination, and high compactness ($\ellhard$ required).
...
Because the kill-shot criterion is intentionally strict, we apply minimal screens to define
``vetted'' high-spin, high-$\lambda$ systems and to handle missing observables consistently:
\begin{titemize}\label{list:screens}
\item \textbf{Spin screen:} prioritize sources with an explicit spin estimate (e.g., Fe~K$\alpha$ reverberation/relativistic reflection, continuum-fitting where applicable, or polarimetric constraints). For Tier-1 we treat $a_\ast\gtrsim0.95$ as ``high spin'' and report the estimator and its systematic caveats.
\item \textbf{Accretion screen:} define $\lambda\equiv L_{\rm bol}/L_{\rm Edd}$ using a documented $L_{\rm bol}$ method (SED-based where available) and a stated $M_{\rm BH}$ estimator; propagate conservative systematics (bolometric correction and virial-mass uncertainties).
\item \textbf{Geometry/obscuration screen:} exclude Compton-thick or heavily absorbed cases where spectral curvature, lags, or polarization become instrumentally ambiguous. If an observable is effectively inaccessible at useful S/N, treat it as \emph{missing} rather than \emph{failed}.
\item \textbf{Compactness screen:} exclude pair-opaque states ($\ell\gtrsim\ellcrit$) where the hard component is suppressed; treat such cases as \emph{screened} rather than \emph{failed}.
\end{titemize}
...
\noindent\textbf{Hooks (apply jointly under Screens S1--S4).}
...
\begin{titemize}
  \item[(i)] \textbf{Hard X/MeV shoulder:} a hard excess/shoulder beyond a featureless baseline continuum.
  \item[(ii)] \textbf{Lag hardening:} a lag--energy relation consistent with harder photons lagging softer photons within an epoch.
  \item[(iii)] \textbf{Equator-aligned X-ray polarization:} EVPA preferentially aligned with the disk plane, with a trend toward higher polarization degree at higher spin/high $\lambda$.
\end{titemize}
\noindent\textbf{Lag convention.}
We measure lags relative to a fixed reference band (stated explicitly for each dataset), and adopt $\tau>0$ when the target band lags the reference.

\begin{quote}
\noindent\textbf{Observer Quickstart (Tier-1 archival screening; non-committal).}\label{box:quickstart}\par
\small
\begin{tenumerate}
\item[\textbf{S1.}] \textbf{Screen first:} apply the screens above (spin/accretion/geometry/compactness) and document any missing observables.
\item[\textbf{S2.}] \textbf{Hook 1 (spectral):} perform the model comparison specified in Table~\ref{tab:hook-spec} and report the preference metric(s).
\item[\textbf{S3.}] \textbf{Hook 2 (timing):} compute the lag--energy relation and report the sign/monotonicity criterion specified in Table~\ref{tab:hook-spec}.
\item[\textbf{S4.}] \textbf{Hook 3 (polarization):} test the EVPA alignment criterion as specified in Table~\ref{tab:hook-spec}.
\item \textbf{Decision:} Tier-1 passes motivate Tier-2 follow-up; persistent null results in vetted high-spin/high-$\lambda$, radio-quiet samples contribute directly to falsification.
\end{tenumerate}
\end{quote}

\noindent\textbf{Falsification (explicit criterion).}
Under Screens S1--S4, the absence of two or more hooks in vetted high-spin/high-$\lambda$ samples falsifies the SSPE contribution; otherwise the source passes the Tier-1 screen.
...
Hook 2 & Lag hardening & lag vs.\ $E$ slope &  $d\tau/d\log E>0$ and monotone within an epoch. & dilution\\
Hook 3 & Equatorial polarization & 2--10 keV & EVPA $\parallel$ disk & jet contamination\\
\enddata
\tablecomments{after applying Screens S1--S4; failure to recover $\ge2$ hooks falsifies SSPE.}
\end{deluxetable*}

%\FloatBarrier
%\floattable
%\begin{deluxetable*}{lccc}[t] % ← ★付き＋[t]で上部固定（専用ページなら [p]）
%\tablecaption{\textbf{At a glance:} How power repartitions at high $\lambda$ across mechanisms.\newline \textbf{Bottom line:} The equatorial channel sustains $L/L_{\rm Edd}\!>\!1$ \emph{and} imposes a spin ceiling $a_{\rm eq}\!\approx\!a_{\rm th}$, complementary to slim-disk \citep{Abramowicz1988} (no ceiling) and BZ/MAD (polar power, $a_{\rm eq}\!\sim\!0.5$--$0.7$).\label{tab:mech-contrast}}
%\tabletypesize{\small}
%\tablewidth{0pt}
%\tablehead{
...
SSPE is offered as such a channel in a form that can be rejected decisively: after applying the Screens S1--S4, the high-spin/high-$\lambda$ subset should recover at least two of the three hooks (Sec.~\ref{sec:obs}); systematic failure would support $f_{\rm eq}\!\to\!0$ and render SSPE unnecessary.
...
Figure~\ref{fig:param-a}--\ref{fig:param-b} illustrates how the Screen S4
...
\textbf{Partly} &
\mcell{After screens, lag--energy slopes\\fail to be  $d\tau/d\log E>0$ and monotone within an epoch\\across the bright subset}\\
\tableline
Hook 3 &
...
\begin{enumerate}
  ...
  \item[\textbf{(ii)}] \textem{\textbf{Lag hardening.}} Within single epochs, verify that X$\to$UV/optical lags increase monotonically with photon energy; mask absorption events; provide cadence/noise tests.
...
\noindent\textbf{Screens S1–S4 (self-contained)} Exclude Compton-thick, extreme inclination; require $\ell<\ellcrit$.\\
\noindent\textbf{Hook (i):} Shoulder passes if $\ICmetric \ge \ICcrit$ (20--120~keV obs).\\
...
\textem{Toy mapping:} see Fig.~\ref{fig:fig_appendix}.
