...
\begin{equation}
\max\!\left(
\frac{\left|HR - HR_0\right|}{\sigma_{HR}},\ 
\frac{\left|F - F_0\right|}{\sigma_F}
\right)\ \le\ 2,
\label{eq:epoch-constancy}
\end{equation}
over the block, where $(HR_0,F_0)$ are the block means and $(\sigma_{\rm HR},\sigma_F)$ are the $1\sigma$ uncertainties of the individual measurements (not the standard error of the block mean), including instrumental noise.

Operationally, Eq.~(\ref{eq:epoch-constancy}) is applied to the in-block measurements under the fixed reduction/selection procedure, and the constancy requirement is enforced for the bulk of points (e.g., at least 95\%) to ensure robustness to occasional outliers.
