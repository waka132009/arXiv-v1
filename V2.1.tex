...
\tableline
Hook 2 & Lag hardening & lag--energy slope & \mccell{$\tau(E)$ relative to $E_{\rm ref}=2$--4~keV \\rest-frame; use $(E_{\rm ref}/(1+z))$\\in observed frame where feasible\\ define $\tau>0$ for lags; \\ pass if $d\tau/d\log E>0$ \\ and monotone\tablenotemark{b} within an epoch\tablenotemark{a}.} & dilution\\
\tableline
Hook 3 & Equatorial polarization & 2--10 keV & EVPA $\parallel$ disk (equatorial plane) & jet contamination\\
\enddata
\tablecomments{
After applying Screens~A--D, each hook may retain degeneracies in isolation; SSPE is therefore evaluated by joint coherence across Hooks~1--3.\\
For Hook 1, We adopt $\ICmetric \ge \ICcrit$ as a pragmatic strong-evidence threshold for within-object model discrimination (with $\AICcmetric \ge \ICcrit$ as a secondary check) following common usage in model selection.\\
Appendix~\ref{app:protocol} does not perform population inference; instead, it defines a screened-domain existence-style criterion based on whether the SSPE-like joint-hook pattern is recovered in more than one source after screening.
\\[1ex]
Screen~D acts as an observability gate \textbf{(screened $\neq$ failed)}: sources outside the transparency domain are excluded from the current archival Tier-1 existence bookkeeping due to instrumental sensitivity limits, but remain primary targets for future MeV-regime Tier-2 tests.
\\[1ex]
Table~\ref{tab:hook-spec} defines per-hook adjudication (pass/fail/missing), while Appendix~\ref{app:protocol} defines the object-level bookkeeping and the screened-domain existence criterion.
}
...
\tablenotetext{b}{Here “monotone” means $\tau(E)$ is non-decreasing across the adopted energy bins within the quoted uncertainties (i.e., no statistically significant reversals).}
\end{deluxetable*}
...
If independent evidence suggests that $f_{\rm oth}$ is not small or is rapidly variable, the see-saw diagnostic is treated as non-decidable for that object/epoch and is not used as a falsifier.

Importantly, $f_{\rm oth}$ does not modify the Tier-1 decision logic: for screened, vetted objects with decidable joint-hook outcomes, an object’s failure to recover $\ge 2$ \emph{decidable} hooks refutes SSPE, independent of how the extracted power is partitioned into residual channels ($P_{\rm oth}$).
...
This speculative leakage scenario is not required for the protocol and does not affect the falsification logic in Table~\ref{tab:roadmap} (decision bookkeeping).
...
%\noindent Falsification is triggered when
%\begin{equation}\label{eq:PF1}
%N_{\rm fail}\ \ge\ N_{\rm crit}(N_{\rm eff},\alpha),
%\end{equation}
%#where $N_{\rm crit}$ is the smallest integer satisfying the one-sided condition
%\begin{equation}\label{eq:PF2}
%P\!\left(X \ge N_{\rm crit}\ \middle|\ X\sim{\rm Binom}(N_{\rm eff},p_0{=}0.5)\right)\ \le\ \alpha.
%\end{equation}
%Missing sources are excluded from the denominator when computing the population-level failure rate, i.e., $N_{\rm eff}\equiv N_{\rm total}-N_{\rm miss}$.
