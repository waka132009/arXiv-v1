...
\phead{Population-level falsification (Tier-1).}
Let $N_{\rm eff}\equiv N_{\rm total}-N_{\rm miss}$ be the effective sample size and $N_{\rm fail}$ the number of adjudicated Tier-1 failures.

\phead{Single source of truth (Tier-1).—} Eqs.~(\ref{eq:PF1}--\ref{eq:PF2}) define the operational population-level trigger; any $(N_{\min}, f_{\rm fail})$ phrasing elsewhere is a heuristic summary for a given $(N_{\rm eff},\alpha)$.
