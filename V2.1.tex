...
and $2$--$3\,\LEdd$ when combined with concurrent accretion, while $da_*/dt<0$ prevents overspin.
...
\textit{Tier-1 archival test:} if the screened high-spin/high-$\lambda$ archival set does not recover the joint-hook pattern in more than one source, the existence-style claim is not established in that regime.}
In this paper we treat SSPE as a concrete, operationally testable working framework rather than a completed theory.


This work does not adjudicate between GRMHD/FFE/PIC implementations; it treats an equatorial channel as a operational boundary condition and targets observational joint coherence.
...
To prevent post-hoc reclassification, we predefine an operational floor for each Hook in Table~\ref{tab:hook-spec}; below this floor the outcome is \emph{missing} by construction and does not does not enter Tier-1 adjudication.
...
We therefore label $\ell>\ell_{\rm crit}$ sources as \emph{screened / untestable with the present low-cost protocol}, not as failures under Tier-1 bookkeeping.
...
If the screened archival set recovers the joint-hook pattern in more than one source, the Tier-1 existence-style claim is supported and Tier-2 follow-up is motivated.\\
...
Under Screens A--D, Tier-1 asks whether the SSPE-like joint-hook pattern is recovered in more than one screened source within the vetted high-spin/high-$\lambda$ archival set.
