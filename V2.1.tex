...
Signatures detected with comparable frequency in low-spin control samples (See Scope~\ref{sec:scope-def}) would indicate a spin-insensitive origin (general disk/corona physics) and are therefore \emph{not} interpreted as SSPE in this work, without altering the protocol’s Tier-1 decision bookkeeping.
%\end{samepage}
...
\begin{enumerate}
  \item[\textbf{(i)}] \textem{\textbf{Polarization vs.\ spin.}}Test that EVPA is equator-aligned and that the degree rises with $a_*$/$\lambda$; control for jet and scattering-cone geometries; report rank-correlation $p$-values and effect sizes.
...
Using Sec.~\ref{sec:power} as the bookkeeping ($P_{\rm ext}=P_{\rm BZ}+P_{\rm eq}+P_{\rm oth}$; $f_{\rm BZ}+f_{\rm eq}+f_{\rm oth}=1$), test whether states (or sources) with larger jet-dominance proxies (e.g., radio loudness, core dominance, or independent jet-power estimates) systematically show weaker equatorial reheating diagnostics (Hooks~1--3),at fixed spin/$\lambda$ and after screens.
...
\begin{enumerate}
  ...
  \item[\textbf{(iii)}] \textem{Population modeling.} As a follow-up exercise beyond the present Tier-1 scope, combine spin distributions with transparency and coupling priors to estimate the fraction of sources expected to satisfy the joint-hook requirement.
\end{enumerate}
...
If two or more vetted sources satisfy the joint-hook pass condition, the Tier-1 existence-style claim is supported.
...
\phead{Counts and units (Tier-1).}
All population-level tallies are at the \emph{object} level unless stated otherwise.
\begin{quote}
$N_{\rm total}$: number of \emph{objects} in the vetted sample.\\
\noindent\emph{Vetted sample.—} In Tier-1, $N_{\rm total}$ counts objects that pass Screens~A--D; objects screened out by any screen (including Screen~D) are not included in $N_{\rm total}$.\\
$N_{\rm miss}$: number of \emph{objects} for which the joint-hook rule is non-decidable (fewer than two adjudicable hook outcomes under Table~\ref{tab:hook-spec}).\\
$N_{\rm eff}\equiv N_{\rm total}-N_{\rm miss}$: effective number of \emph{objects} entering the Tier-1 population test.\\
$N_{\rm pass}$: number of \emph{objects} in the effective sample that satisfy the joint-hook requirement.\\
$N_{\rm fail}$: number of \emph{objects} that fail the joint-hook requirement among the effective sample.\\
%$f_{\rm fail}\equiv N_{\rm fail}/N_{\rm eff}$.
\end{quote}


\phead{Single source of truth (Tier-1).}
Tier-1 is an existence-style gate within the screened domain, not a population-inference test.

%Let $N_{\rm eff} \equiv N_{\rm total}-N_{\rm miss}$ be the effective number of objects entering Tier-1 adjudication, and let $N_{\rm pass}$ be the number of objects that satisfy the joint-hook requirement among that effective sample.

\begin{equation}
N_{\rm pass} \ge 2 ,
\label{eq:tier1-existence}
\end{equation}

\noindent
establishes the existence-style claim advanced in this work, namely that the SSPE-like joint-hook pattern occurs in more than a single isolated object within the screened domain.

If $N_{\rm pass}\le 1$ after applying the pre-registered screens and bookkeeping, the existence-style claim is not established.

If the available sample is too small or too incomplete for a meaningful Tier-1 adjudication, the result is treated as inconclusive rather than supportive.

If the effective non-missing sample $N_{\rm eff}$ is too small for meaningful Tier-1 adjudication, the result is treated as inconclusive rather than supportive.


\phead{Note (data quality).}
If a Tier-1 hook cannot be adjudicated as pass/fail at useful S/N under the fixed floors in Table~\ref{tab:hook-spec} (given the stated uncertainties/limits), its outcome is treated as \emph{missing} by construction (not failed) and is excluded from the Tier-1 bookkeeping.
...
\noindent\textbf{Decision (Tier-1 bookkeeping).}—
Tier-1 adjudication is evaluated using the screened-domain existence criterion together with the missing/fail bookkeeping defined above; see Appendix~\ref{app:protocol}.
