...
\tableline
Hook 2 & Lag hardening & lag--energy slope & \mccell{$\tau(E)$ relative to $E_{\rm ref}=2$--4~keV \\rest-frame; use /(1+z)\\in observed frame where feasible\\ define $\tau>0$ for lags; \\ pass if $d\tau/d\log E>0$ \\ and monotone within an epoch(See~App.\ref{app:protocol}).} & dilution\\
\tableline
...
SSPE predicts that the joint-hook strength should correlate with spin-linked quantities (e.g., the equilibrium spin trends and iron-line diagnostics), whereas a purely geometric corona interpretation lacks a generic mechanism to lock the same correlations without additional tuning.

In SSPE the heating channel is ultimately powered by black-hole rotational energy extraction, so the joint-hook strength is expected to correlate with spin-linked quantities at fixed $(M_{\rm BH},\lambda)$.
...
We therefore treat spin-linked scaling (e.g., via reflection/Fe K diagnostics; Fig.~\ref{fig:fig_appendix}) as a Tier-2 discriminator.
...
\FloatBarrier
\phead{Primary falsifier (population-level).}SSPE is refuted if a vetted sample of at least $N_{\min}$ independent objects fails the joint-hook requirement in more than a fraction $f_{\rm fail}$ of the sample (under Screens~A--D and the operational floors), i.e., fails to recover $\ge2$ hooks within same epoch.

We adopt $N_{\min}=10$ and $f_{\rm fail}=0.5$ as pragmatic defaults.
...
\item $L_{\rm bol}$ not anti-correlated (even weakly) with $a_\ast$ within matched bins of (MBH, $\lambda$) the bright subset (no self-regulated spin-down imprint).
...
\noindent\textbf{Hook 2 (lag hardening):} passes if $d\tau/d\log E > 0$ and $\tau(E)$ is monotone within \emph{an epoch}.\\
