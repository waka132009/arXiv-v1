...
%\tldr{Total luminosity combines accretion and equatorial feedback, generically yielding $L/L_{\rm Edd}\sim2$--$3$.}
...
Figure~\ref{fig:fig5} summarizes how the equilibrium spin $a_{\rm eq}$ maps onto two directly testable observables used in our hook protocol.
\begin{figure*}[t]
  \centering
...
\noindent
The semi-analytic exploration above yields three robust trends that do not depend on the microphysical details of how the equatorial branch is realized.
...
\noindent
These trends map directly onto the observational program in Sec.~\ref{sec:hooks}.
...
\small
\begin{tenumerate}
\item \textbf{Hook 1 (spectral):} fit a baseline (e.g., cutoff power-law) and a shoulder-enabled alternative over 20--120~keV, and report preference using $\Delta\mathrm{BIC}$ or $\Delta\mathrm{AICc}$ (threshold: $\ge 6$).
\item \textbf{Hook 2 (timing):} compute the lag--energy relation in the same epoch and test only the sign/monotonicity, requiring $d(\mathrm{lag})/d\log E>0$ over the adopted band.
\item \textbf{Decision:} sources satisfying both Tier-1 hooks motivate Tier-2 follow-up; persistent null results in vetted high-spin/high-$\lambda$, radio-quiet samples contribute directly to falsification under Sec.~\ref{sec:obs}.
\end{tenumerate}
\end{quote}
% --- 図はトップ優先で近くに出す ---
%\Needspace{22\baselineskip}
%\begin{figure}[t!]\label{fig:fig5}
%  \vspace*{2pt}
%  \centering
%  \includegraphics[width=.95\linewidth]{Fig5a.pdf}\\[4pt]
%  \includegraphics[width=.95\linewidth]{Fig5b.pdf}
%  \caption{\textbf{Hook observables vs.\ equilibrium spin.}
%  \emph{Top:} Fe K$\alpha$ reverberation lag $\tau_{\rm FeK}$ (in $r_g/c$) as a function of $a_{\rm eq}$; lags increase toward lower $a_{\rm eq}$ (stronger coupling).
%  \emph{Bottom:} Observable fraction $f_{\rm hook}$ (hard X/MeV shoulder or 2--10~keV polarization) vs.\ $a_{\rm eq}$ from the toy population mapping; the fraction rises steeply toward low $a_{\rm eq}$.
%  See Sec.~\ref{sec:obs} and App.~\ref{app:protocol} for definitions and pass criteria.}
%\end{figure}
...
\tldr{The equatorial branch complements polar BZ; an anti-correlated partition is expected across states.}
