...
\paragraph{Power partition.}—We allow the extracted rotational power to partition into a polar Blandford--Znajek (BZ) channel, an equatorial feedback channel, and other loss channels:
\[
...
\begin{titemize}
\item \textbf{Spin screen:} prioritize sources with an explicit spin estimate (e.g., Fe~K$\alpha$ reverberation/relativistic reflection, continuum-fitting where applicable, or polarimetric constraints). For Tier-1 we treat $a_\ast\gtrsim0.95$ as ``high spin'' and report the estimator and its systematic caveats.We emphasize that this is an operational screen rather than a sharp physical boundary: the goal is to avoid boundary disputes in Tier-1, while sources near the threshold can be revisited in Tier-2 with improved systematics.
\item \textbf{Accretion screen:} define $\lambda\equiv L_{\rm bol}/L_{\rm Edd}$ using a documented $L_{\rm bol}$ method (SED-based where available) and a stated $M_{\rm BH}$ estimator; propagate a conservative systematic uncertainty (e.g., bolometric correction and virial mass systematics).
\item \textbf{Geometry/obscuration screen:} exclude Compton-thick or heavily absorbed cases for which spectral curvature, lags, or polarization become instrumentally ambiguous. If an observable is effectively inaccessible (not measurable at useful S/N), treat it as \emph{missing} rather than \emph{failed}.
\end{titemize}

\noindent We emphasize that the observational hooks and thresholds were specified \emph{a priori} as a falsification-oriented protocol, and only subsequently confronted with existing reports and archival indicators to assess feasibility. This ordering is intended to minimize post hoc tuning and to keep the framework portable across instruments and subfields.
...
\end{deluxetable*}
\noindent\textem{Why this matters.} What to look for and how to falsify, in plain terms.

% --- Quick recall of the equations each plot is tied to (往復を減らす) ---
\noindent\textem{Recall.}
...
\begin{titemize}
  \item[(1)] \textbf{Hard X/MeV shoulder:}
        evidence for a hard excess/shoulder beyond a featureless baseline continuum.
  \item[(2)] \textbf{Lag hardening:}
        a lag--energy relation consistent with harder photons lagging softer photons within an epoch.
  \item[(3)] \textbf{Equator-aligned X-ray polarization:}
        EVPA preferentially aligned with the disk plane, with a trend toward higher polarization degree at higher spin/high $\lambda$.
        We note that wind-scattering models can also polarize the X-ray continuum (e.g., \citealt{Young2011AGNWindPol}), but they rely on a distinct geometry and typically predict different inclination/energy dependencies; our screening explicitly targets low-obscuration, radio-quiet systems to reduce such degeneracies.
\end{titemize}
...
\small
\begin{tenumerate}
\item \textbf{Screen first:} apply the exclusion criteria (obscuration/geometry/compactness) as defined above.
\item \textbf{Hook 1 (spectral):} perform the model comparison specified in Table~\ref{tab:hook-spec} and report the preference metric(s).
\item \textbf{Hook 2 (timing):} compute the lag--energy relation and report the sign/monotonicity criterion specified in Table~\ref{tab:hook-spec}.
\item \textbf{Hook 3 (polarization):} test the EVPA alignment criterion as specified in Table~\ref{tab:hook-spec}.
...
Figure~\ref{fig:param-a}--\ref{fig:param-b} illustrates how the compactness screen
