...
\smallskip
\phead{Epoch definition (operational).}We define an \emph{epoch} as a single, quasi-stationary observing block during which(i) the hardness ratio and (ii) the broadband flux, computed with fixed band definitions and a fixed reduction/selection procedure within the block.\\
...
For the population-level decision rule (including $N_{\min}$, $f_{\rm fail}$, and the definition of “decidable”), see the Primary falsifier (population-level) on p.~\pageref{ph:primary-falsifier}.
