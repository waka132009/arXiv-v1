...
The microphysics of the coupling between the ergoregion, quasi-equatorial beams, and the inner disk is kept deliberately agnostic and absorbed into a small set of phenomenological parameters.
What is fixed --- and offered to observers and simulators --- are three testable hooks and an explicit kill-shot:
(1) a hard X/MeV ``shoulder'' in the $\sim$20--120~keV band (Hook~1),
(2) lag hardening in Fourier-resolved timing, in which harder bands lead softer bands over a well-defined frequency range (Hook~2), and
(3) X-ray polarization preferentially aligned with the disk plane in the hard component (Hook~3).
These are complemented by a population-level kill-shot: high-spin, high-Eddington-ratio, radio-quiet quasars must not \emph{systematically} lack Hooks~1--3.
The goal of this paper is therefore not to close the question, but to make it straightforward for the community to confirm, refine, or decisively rule out SSPE using existing and near-future data.
