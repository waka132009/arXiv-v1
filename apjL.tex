...
\begin{figure*}[htbp]
  \centering
  \includegraphics[width=.9\textwidth]{sspe_infographic_p1_conundrum.pdf}
\caption{\textbf{The Quasar Conundrum.}
  ...
  \label{fig:p1} \label{fig:conundrum}
\end{figure*}
...
\noindent\textbf{Observability Gate.} For the extracted power $L_{\rm self}$ to escape, the dissipation region must remain transparent to pair production . This requires the pair compactness $\ell$ to remain low:
\begin{equation}
\ell=\frac{L_{\rm self}\sigma_T}{4\pi R_{\rm eff}\,m_e c^3}\lesssim 30,
\label{eq:ell}
\end{equation}
which implies an extended dissipation footprint ($R_{\rm eff}\sim10^2$--$10^3\,\rg$). This compactness limit acts as a physical gate on the observable luminosity.
...
\begin{figure}[!htbp]
  ...
  Peak $\Ltot/\LEdd$ (from Eq.~\ref{eq:Ltot_simple}) rises rapidly once $a_\ast\!>\!\ath$ (fiducial $\ath=0.97$), setting the lever for both luminosity and spin regulation.}
  \label{fig:onset}
\end{figure}
...
\begin{figure*}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{sspe_infographic_p3_hooks.pdf}
...
We highlight three testable signatures (Fig.~\ref{fig:hooks}) expected to co-occur at high spin ($a_*\to 1$) and high accretion rate ($L/L_{\rm Edd}>1$) under the transparency condition ($\ell \lesssim 30$).

\noindent\textbf{Screens (apply first):}
Exclude heavy absorption (Compton-thick etc.), extreme inclination, and high-compactness cases ($\ell\gtrsim\ellcrit$).
...
\begin{titemize}
  ...
  \item \textbf{Equator-aligned X-ray polarization (2--10~keV):}
        EVPA $\parallel$ equator;
        polarization degree increases toward high spin/high $\lambda$.
\end{titemize}

\noindent\textbf{Null Criteria (Falsification):}
In vetted high-spin, high-$\lambda$ quasar samples, \emph{failure to recover at least two hooks} after applying the screens \emph{refutes} this framework.
...
We model the total extracted spin power as a partition $P_{\rm ext} = P_{\rm BZ} + P_{\rm eq}$, where the fractions $f_{\rm BZ} + f_{\rm eq} = 1$.
This framework naturally predicts an anti-correlated "see-saw" between polar (jet) power and equatorial (radiative) signatures within the high-spin, high-$\lambda$ subset.
While BZ ($f_{\rm BZ}$) may dominate in radio-loud systems, we claim $f_{\rm eq}$ is statistically non-zero and often dominant in the radio-quiet, radiatively-dominated states that require $L/L_{\rm Edd}>1$.
% ---- 5. Conclusions ----
...
The framework is explicitly falsifiable: in vetted high-spin, high-$\lambda$ samples, the systematic absence of at least two of the three proposed observational hooks (a 20--120 keV shoulder, X-to-UV lag hardening, and equator-aligned polarization) would refute this scenario.
