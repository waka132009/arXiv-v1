...
\begin{figure}[t!]
  ...
  \caption{\textbf{Empirical hooks implied by Eqs.~(2)-(11).} 
...
\begin{figure*}[t!]
  \centering
  \plottwo{fig10a_Reff100.pdf}{fig10b_Reff1000.pdf}
  \caption{\textbf{Parameter space implied by Eq.~\eqref{eq:eps} and the compactness constraint Eq.~\eqref{eq:ell}.}
  Left: $R_{\rm eff}=100\,\rg$. Right: $R_{\rm eff}=1000\,\rg$.
  Color encodes the \emph{peak} $\Ltot/\LEdd$; the black contour marks $\ell{=}30$ (pair-compactness threshold).
  Viable solutions lie \emph{outside} the contour; bright but compact regions are disfavored.
  Fiducials: $M=10^9M_\odot$, $\eta_{\rm acc}=0.1$, $\epsilon_{\max}=0.1$, $n=2$, $R_0=10^2$, $\alpha=50$.}
  \label{fig:param-space}
\end{figure*}

\section{Discussion}\label{sec:discussion}
These bounds align with Fig.~\ref{fig:param-space} and are sufficient to reach $L/\LEdd\sim2$--$3$ with rarer $5$--$10\times$ excursions. Slim disks allow modest super-Eddington flows but no spin ceiling. BZ/MAD explain jet power yet not the radiative dominance of radio-quiet quasars. This framework ties hyperluminous output and the spin ceiling via near-extremal, equatorial coupling.
...
% ---- 両段フロートの上部占有を少し緩めて、下に本文が入る余白を作る ----
\makeatletter
\setcounter{dbltopnumber}{2}
...
% 表の密度と横幅を調整（このブロック内だけ有効）
\floattable
\begingroup
\setlength{\tabcolsep}{5.0pt}   % 列間ちょい詰め（既定6pt）
\def\arraystretch{1.08}         % 行間ほんの少しだけ（読みやすさは維持）
...
We highlight three testable signatures expected to co-occur at high spin and high Eddington ratio under transparency ($\ell\!\lesssim\!30$):
\begin{itemize}
\item \textbf{Hard-X shoulder}: a high-energy excess favored over a featureless continuum by $\Delta$AICc/BIC $\ge 6$ in 20--120 keV (obs).
