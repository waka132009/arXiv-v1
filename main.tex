...
\newcommand{\figcallout}[2]{%
  ...
  \textbf{Figure~\ref{#1}} \textemdash{} #2\par
  \endgroup\vspace{2pt}%
}
...
\begin{figure}[!htbp]
  ...
  \emph{Bottom:} Equilibrium spin $a_{\rm eq}$ obtained from $da_\ast/dt=0$ as a function of $\epsilon_{\rm coup}$; the shaded band $\epsilon_{\rm coup}\sim10^{-2}$--$10^{-1}$ is the fiducial range used elsewhere.
See also \eqref{eq:OmegaH}.}  \label{fig:fig3}
\end{figure}
...
\begin{figure}[!htbp]
  ...
  $a_{\rm eq}<1$ illustrates the self-limiting ceiling; see Sec.~\ref{sec:discussion}.}  \label{fig:spin-evo}
\end{figure}
\end{CrowdedFloats}
%\FloatBarrier
...
\begin{figure}[t!]
  ...
  See Sec.~\ref{sec:hooks} and App.~\ref{app:protocol} for definitions and pass criteria.}
  \label{fig:fig5}
\end{figure}
