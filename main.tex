...
\begin{abstract}
Quasars often display luminosities exceeding the Eddington limit of their central SMBHs by factors of several to ten. We propose a self-sustained Penrose excitation mechanism that couples near-horizon equatorial structures to the ergoregion as the spin parameter $a_\ast$ approaches unity, injecting power into the disk while extracting angular momentum. Even modest effective coupling ($\epscoup\sim0.01$--$0.1$) yields $2$--$3\times \LEdd$ on average, with occasional $5$--$10\times$ episodes when the effective dissipation radius $R_{\rm eff}$ is sufficiently large to remain transparent. We adopt a phenomenological framework and do not fix the microphysics; results depend on a small set of effective parameters. Unless noted, all plotted luminosities are \textit{isotropic-equivalent}. Crucially, the proposed excitation is confined to the ergoregion and the equatorial disk—i.e., it is a horizon-exterior mechanism and thus, in principle, empirically testable.
We explicitly identify the ISCO–ergoregion interface as a \emph{leaky} equatorial return path where plasmoid-dominated reconnection acts as a magnetically assisted Penrose trigger, venting power as a narrow equatorial quasi-beam that heats the inner disk and self-regulates spin.
To our knowledge, this is the first framework that \emph{unifies} super-Eddington luminosity with the near-extremal spin ceiling via an \emph{equatorial, magnetically assisted Penrose split} at the ISCO–ergoregion interface, venting power as a narrow quasi-beam that reheats the inner disk and self-regulates spin.
\end{abstract}

\keywords{Quasars --- Accretion, accretion disks --- Black hole physics --- Relativistic processes}

\paragraph{Context.}
Recent reviews have noted persistent tensions between thin–disk expectations and observations—e.g., systematically large continuum sizes and wavelength-dependent lags exceeding simple reprocessing models \citep[e.g.,]{REV1,REV2}.
These tensions motivate considering \emph{additional inner-disk heating} beyond local viscous dissipation.
Near $a_*\!\to\!1$, our \emph{equatorial, magnetically assisted Penrose-like split} provides a \emph{testable route} to such reheating (Sec.~\ref{sec:mech-sketch}), with observational hooks and falsification criteria (Sec.~\ref{sec:obs-hooks}).
...
\begin{figure*}
  \centering
  \includegraphics[width=0.9\textwidth]{fig_equatorial_trigger_schematic.pdf}
...
\begin{figure}[!t]
  ...
  \caption{Empirical hooks implied by Eqs.~\eqref{eq:eps}--\eqref{eq:Ltot}. \textbf{(a)} Fe K$\alpha$ lag (in $r_g/c$) versus $\aeq$ (dimensionless). \textbf{(b)} Observable fraction (hard X/MeV or polarization; dimensionless) versus $\aeq$.}
  \label{fig:fig5}
\end{figure}
\begin{table}[t]
\centering
\caption{Mechanism contrast (schematic).}
\begin{tabular}{lccc}
\hline
 & $L/L_{\rm Edd}>1$ & spin ceiling & channel \\
\hline
Slim disk & yes (advective) & no & radiation (disk) \\
BZ/MAD    & indirect & yes ($a_{\rm eq}\!\sim\!0.5$–$0.7$) & polar Poynting (jet) \\
...
\[
P_{\rm ext} = P_{\rm jet}+P_{\rm eq},\quad
P_{\rm jet}=f_{\rm BZ}\,P_{\rm ext},\quad
P_{\rm eq}=f_{\rm eq}\,P_{\rm ext},\quad
f_{\rm BZ}+f_{\rm eq}=1,\; 0\le f_{\rm BZ},f_{\rm eq}\le1.
\]
